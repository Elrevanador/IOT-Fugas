...
- `POST /api/auth/login` y `POST /api/auth/register` tienen rate limit por IP en memoria.
...
- `DB_RUN_MIGRATIONS=true` ejecuta migraciones aditivas al arrancar. `DB_USE_SYNC=true` mantiene compatibilidad con el bootstrap actual basado en Sequelize.
- La migracion `20260420_0001_init_schema.js` crea el esquema base completo para bases nuevas. Con una base ya inicializada y validada, puedes empezar a probar `DB_USE_SYNC=false`.
